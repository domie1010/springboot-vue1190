		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"width":"80%","margin":"0 0 20px","alignItems":"flex-start","flexDirection":"column","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"border":"1px solid #654B3C","width":"auto","overflow":"hidden","borderRadius":"20px","display":"inline-block"}' >
					...
					<div :style='{"margin":"0 0 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">申请周次</label>
						<el-input v-model="searchForm.shenqingzhouci" placeholder="申请周次" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 0 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">申请星期</label>
						<el-input v-model="searchForm.shenqingxingqi" placeholder="申请星期" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 0 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">申请节次</label>
						<el-input v-model="searchForm.shenqingjieci" placeholder="申请节次" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"width":"100%","margin":"20px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('xueshengshenqing','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#654B3C","height":"40px"}'></span>
						创建
					</el-button>
					<el-button class="del" v-if="isAuth('xueshengshenqing','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu10" :style='{"margin":"0 2px","fontSize":"14px","color":"#654B3C","height":"40px"}'></span>
						删减
					</el-button>


					<el-button class="btn18" v-if="isAuth('xueshengshenqing','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
			...
			<div :style='{"width":"100%","padding":"10px","boxShadow":""}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('xueshengshenqing','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"14px","color":"#965F40","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('xueshengshenqing','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('xueshengshenqing','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import AddOrUpdate from "./add-or-update";
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.shenqingzhouci!='' && this.searchForm.shenqingzhouci!=undefined){
            params['shenqingzhouci'] = '%' + this.searchForm.shenqingzhouci + '%'
          }
           if(this.searchForm.shenqingxingqi!='' && this.searchForm.shenqingxingqi!=undefined){
            params['shenqingxingqi'] = '%' + this.searchForm.shenqingxingqi + '%'
          }
           if(this.searchForm.shenqingjieci!='' && this.searchForm.shenqingjieci!=undefined){
            params['shenqingjieci'] = '%' + this.searchForm.shenqingjieci + '%'
          }
    ...
    shBatchHandler(){
      this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
        ...
        this.$http({
          url: "xueshengshenqing/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
          method: "post",
          data: this.batchIds
        }).then(({ data }) => {
          if (data && data.code === 0) {
	...
	xueshengshenqingstatusChange(e,row){
		if(row.status==0){
