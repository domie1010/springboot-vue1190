<template>
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="节次" prop="jieci">
					<el-input v-model="ruleForm.jieci" placeholder="节次" clearable  :readonly="ro.jieci"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="节次" prop="jieci">
					<el-input v-model="ruleForm.jieci" placeholder="节次" readonly></el-input>
				</el-form-item>
			</template>
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `jieci/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jieciCrossAddOrUpdateFlag = false;
											this.parent.search();
