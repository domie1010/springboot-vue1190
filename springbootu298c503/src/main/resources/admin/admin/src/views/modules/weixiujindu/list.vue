			...
			<el-form class="center-form-pv" :style='{"width":"80%","margin":"0 0 20px","alignItems":"flex-start","flexDirection":"column","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"border":"1px solid #654B3C","width":"auto","overflow":"hidden","borderRadius":"20px","display":"inline-block"}' >
					...
					<div :style='{"margin":"0 0 0 0","display":"inline-block"}'>
						...
						<el-input v-model="searchForm.baoxiuriqi" placeholder="报修日期" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 0 0 0","display":"inline-block"}'>
				...
				<el-row class="actions" :style='{"width":"100%","margin":"20px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('weixiujindu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#654B3C","height":"40px"}'></span>
						创建
					</el-button>
					<el-button class="del" v-if="isAuth('weixiujindu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"10px","boxShadow":""}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('weixiujindu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"14px","color":"#965F40","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('weixiujindu','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('weixiujindu','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import AddOrUpdate from "./add-or-update";
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.baoxiuriqi!='' && this.searchForm.baoxiuriqi!=undefined){
            params['baoxiuriqi'] = '%' + this.searchForm.baoxiuriqi + '%'
          }
	...
	weixiujindustatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'weixiujindu/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
