	...
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="报修日期" prop="baoxiuriqi">
					<el-date-picker
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.baoxiuriqi" label="报修日期" prop="baoxiuriqi">
					<el-input v-model="ruleForm.baoxiuriqi" placeholder="报修日期" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="维修状态" prop="weixiuzhuangtai">
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="维修状态" prop="weixiuzhuangtai">
...
export default {
	...
	created() {
		this.ruleForm.baoxiuriqi = this.getCurDateTime()
	},
	methods: {
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `jiaoshibaoxiu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jiaoshibaoxiuCrossAddOrUpdateFlag = false;
											this.parent.search();
